...
local function OnAddonLoaded(event, addonName)
    ...
    if IsConsole() then
        -- Slash commands require a keyboard; unavailable on console.
        -- A settings UI will be added in a future update.
        EVENT_MANAGER:RegisterForEvent(ADDON_NAME .. "_ConsoleNotice", EVENT_PLAYER_ACTIVATED, function()
            EVENT_MANAGER:UnregisterForEvent(ADDON_NAME .. "_ConsoleNotice", EVENT_PLAYER_ACTIVATED)
            local params = CENTER_SCREEN_ANNOUNCE:CreateMessageParams(CSA_CATEGORY_SMALL_TEXT)
            params:SetText("|c3399FFGamePadHelper|r loaded. Settings require a keyboard.")
